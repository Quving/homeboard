...
def get_departuretimes_berliner_tor
    url = ENV["MOBISPRING_URL"]
    uri = URI(url+'/api/geofox/departuretime')
    req = Net::HTTP::Post.new(uri, 'Content-Type' => 'application/json')
    req.body = {"station":"Berliner Tor", "hhMMyyyy":Time.now.strftime("%d.%m.%Y"), "HHmm":Time.now.strftime("%H:%M"), "maxList":"15"}.to_json
...
SCHEDULER.every '20s' do
    hvv_departures = {}
    departures_json = JSON.parse(get_departuretimes_berliner_tor())
    ...
    send_event('hvv_berliner_tor', { items: hvv_departures.values})
end
